...
import {
  parseJsonlLine,
  loadTranscript,
} from "../../../backend/src/parsers/jsonl-parser";
import {
...
describe("Python Test Data Compatibility", () => {
  ...
  describe("representative_messages.jsonl compatibility", () => {
    ...
    test("parses assistant messages correctly", () => {
      const result = loadTranscript(testFilePath, { silent: true });
      const assistantMessages = result.entries.filter(
    ...
    test("parses summary entries correctly", () => {
      ...
      const summaryEntries = result.entries.filter(
        (entry): entry is ISummaryTranscriptEntry => entry.type === "summary",
      );
  ...
  describe("edge_cases.jsonl compatibility", () => {
    ...
    test("handles markdown content correctly", () => {
      ...
      if (
        firstUser &&
        typeof firstUser.message.content === "object" &&
        Array.isArray(firstUser.message.content)
      ) {
        const textContent = firstUser.message.content.find(
          (item) => item.type === "text",
        );
    ...
    test("handles tool use and tool results correctly", () => {
      ...
      if (toolUseMessage && Array.isArray(toolUseMessage.message.content)) {
        const toolUse = toolUseMessage.message.content.find(
      ...
      if (
        toolResultMessage &&
        Array.isArray(toolResultMessage.message.content)
      ) {
        const toolResult = toolResultMessage.message.content.find(
    ...
    test("handles complex MultiEdit tool correctly", () => {
      ...
      const multiEditMessage = assistantMessages.find(
        (msg) =>
          Array.isArray(msg.message.content) &&
      ...
      if (multiEditMessage && Array.isArray(multiEditMessage.message.content)) {
        const toolUse = multiEditMessage.message.content.find(
          (item) =>
            item.type === "tool_use" &&
            "name" in item &&
            item.name === "MultiEdit",
    ...
    testFiles.forEach((filename) => {
      test(`can parse ${filename}`, () => {
        ...
        if (fs.existsSync(testFilePath)) {
          ...
          result.entries.forEach((entry, _index) => {
            expect(entry).toHaveProperty("type");
            expect(["user", "assistant", "summary", "system"]).toContain(
              entry.type,
            );
