  ...
  describe('JSON structure compatibility', () => {
    test('TypeScript parses Python-generated JSONL identically', () => {
      ...
      testFiles.forEach(filename => {
        ...
        const cachePath = path.join(PYTHON_CACHE_DIR, filename.replace('.jsonl', '.json'));
        ...
        if (fs.existsSync(jsonlPath) && fs.existsSync(cachePath)) {
          ...
          const pythonCache = JSON.parse(fs.readFileSync(cachePath, 'utf-8'));
    ...
    test('User message JSON output matches Python format', () => {
      ...
      if (fs.existsSync(testFilePath)) {
        ...
        const userEntries = result.entries.filter((entry): entry is IUserTranscriptEntry => entry.type === 'user');
        
        userEntries.forEach(userEntry => {
          // Convert to Anthropic format and back
          const anthropicFormat = convertUserMessageToAnthropic(userEntry.message);
          const backToInternal = convertAnthropicMessageParamToInternal(anthropicFormat);
    ...
    test('Assistant message JSON output matches Python format', () => {
      ...
      if (fs.existsSync(testFilePath)) {
        const result = loadTranscript(testFilePath, { silent: true });
        const assistantEntries = result.entries.filter((entry): entry is IAssistantTranscriptEntry => entry.type === 'assistant');
        
        assistantEntries.forEach(assistantEntry => {
          // Convert to Anthropic format and back
          const anthropicFormat = convertAssistantMessageToAnthropic(assistantEntry.message);
          const backToInternal = convertAnthropicMessageToInternal(anthropicFormat);
          ...
          if (assistantEntry.message.usage) {
            ...
            if ('usage' in backToInternal && backToInternal.usage) {
              expect(backToInternal.usage.input_tokens).toBe(assistantEntry.message.usage.input_tokens);
              expect(backToInternal.usage.output_tokens).toBe(assistantEntry.message.usage.output_tokens);
            }
          }
        });
      }
      ...
      if (fs.existsSync(testFilePath)) {
        ...
        result.entries.forEach(entry => {
          if ((entry.type === 'user' || entry.type === 'assistant') && 
            ...
            const anthropicContent = convertContentToAnthropic(entry.message.content);
            const backToInternal = convertContentFromAnthropic(anthropicContent);
            ...
            backToInternal.forEach((item, index) => {
              ...
              if (typeof item === 'object' && typeof originalItem === 'object' && 'type' in item && 'type' in originalItem) {
                expect(item.type).toBe(originalItem.type);
              
                if (item.type === 'text' && originalItem.type === 'text' && 'text' in item && 'text' in originalItem) {
                  expect(item.text).toBe(originalItem.text);
                }
                
                if (item.type === 'tool_use' && originalItem.type === 'tool_use' && 'id' in item && 'id' in originalItem) {
                  expect(item.id).toBe(originalItem.id);
                  expect(item.name).toBe(originalItem.name);
                  expect(JSON.stringify(item.input)).toBe(JSON.stringify(originalItem.input));
                }
                
                if (item.type === 'tool_result' && originalItem.type === 'tool_result' && 'tool_use_id' in item && 'tool_use_id' in originalItem) {
            ...
            if (typeof content === 'string') {
              ...
              content.forEach(item => {
                if (item && typeof item === 'object' && 'type' in item) {
                  expect(item.type).toMatch(/^(text|tool_use|tool_result|thinking|image)$/);
  ...
  describe('Error handling parity', () => {
    test('handles same edge cases as Python parser', () => {
      ...
      const edgeCases = [
        ...
        '{"type":"user","uuid":"test","timestamp":"2025-01-15T10:30:00.000Z","sessionId":"session","message":{"role":"user","content":"' + 'x'.repeat(10000) + '"}}',
      ...
      edgeCases.forEach((jsonLine, index) => {
        const result = JSON.parse(jsonLine);
        
        // Should parse without throwing
        expect(() => {
          const parsed = loadTranscript('', { silent: true });
