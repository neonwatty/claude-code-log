...
describe("ClaudeIntegrationService", () => {
  ...
  describe("Session Continuation with Context", () => {
    it("should continue session with context successfully", async () => {
      ...
      const result = await service.continueSessionWithContext(
        contextRequest,
        mockSession,
      );
  ...
  describe("Context Preparation", () => {
    it("should prepare session context successfully", async () => {
      ...
      const result = await service.prepareSessionContext(
        contextRequest,
        mockSession,
      );
  ...
  describe("Event System", () => {
    ...
    it("should support once event subscription", () => {
      ...
      // Emit event twice
      (service as any).eventEmitter.emit("session:continued", {
        sessionId: "test",
        processId: "test",
      });
      (service as any).eventEmitter.emit("session:continued", {
    ...
    it("should support event unsubscription", () => {
      ...
      // Emit event
      (service as any).eventEmitter.emit("error", {
  ...
  describe("WebSocket Integration", () => {
    ...
    it("should handle WebSocket process status updates", () => {
      ...
      const cachedStatus = service.getCachedProcessStatus(
        mockProcessStatus.processId,
      );
  ...
  describe("Cache Management", () => {
    ...
    it("should return all cached processes", () => {
      (service as any).processes.set("process-1", {
        ...mockProcessStatus,
        processId: "process-1",
      });
      (service as any).processes.set("process-2", {
        ...mockProcessStatus,
        processId: "process-2",
      });
    ...
    it("should check if process is running", () => {
      (service as any).processes.set("running-process", {
        ...mockProcessStatus,
        state: "running",
      });
      (service as any).processes.set("stopped-process", {
        ...mockProcessStatus,
        state: "stopped",
      });
  ...
  describe("Utility Functions", () => {
    it("should get session context data", async () => {
      ...
      const result = await service.getSessionContextData(
        "test-session-123",
        mockSession,
      );
