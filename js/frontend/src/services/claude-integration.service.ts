...
export interface ClaudeIntegrationEventMap {
  ...
  'context:prepared': { sessionId: string; claudeMdPath?: string; processingTime?: number };
...
export class ClaudeIntegrationService {
  ...
  private constructor(config: ClaudeIntegrationServiceConfig = {}) {
    ...
    if (this.config.debug) {
      console.log('[ClaudeIntegrationService] Initialized with config:', this.config);
    }
   ...
   */
  public static getInstance(config?: ClaudeIntegrationServiceConfig): ClaudeIntegrationService {
    if (!ClaudeIntegrationService.instance) {
      ClaudeIntegrationService.instance = new ClaudeIntegrationService(config);
    } else if (config) {
      // Update configuration if provided
      ClaudeIntegrationService.instance.config = { ...DEFAULT_CONFIG, ...config };
    }
   ...
   */
  public async continueSession(request: SessionContinuationRequest): Promise<SessionContinuationResponse> {
    try {
  ...
  public async continueSessionWithContext(
    ...
    try {
      ...
      if (response.success) {
        ...
        this.log(`Session continuation with context started: ${response.processId}`);
      }
      
      return response;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.log(`Failed to continue session with context: ${errorMessage}`, 'error');
  ...
  public async prepareSessionContext(
    ...
    try {
      ...
      if (response.success) {
        ...
        this.log(`Context preparation completed for session: ${request.sessionId}`);
      } else {
   ...
   */
  public async getSessionContextData(sessionId: string, session: ZodSession): Promise<ContextPreparationResult> {
    try {
   ...
   */
  public async getProcessStatus(processId: string): Promise<ClaudeProcessStatus | null> {
    try {
  ...
  private setupWebSocketListeners(): void {
    try {
      ...
      this.webSocketUnsubscribers.push(statusUnsubscriber, outputUnsubscriber, errorUnsubscriber);
 ...
 */
export function getClaudeIntegrationService(config?: ClaudeIntegrationServiceConfig): ClaudeIntegrationService {
