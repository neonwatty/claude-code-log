...
export class SimpleWebSocketExample extends BaseComponent {
  ...
  protected override firstUpdated(changedProperties: Map<string | number | symbol, unknown>): void {
  ...
  private setupWebSocketHandlers(): void {
    ...
    this.webSocketController.onSessionCreated((session: SessionData) => {
      ...
      // Visual feedback
      setTimeout(() => {
        const newSessionElement = this.shadowRoot?.querySelector('.session-item:last-child');
    ...
    this.webSocketController.onSessionUpdated((session: SessionData, changes) => {
      ...
      // Visual feedback
      setTimeout(() => {
        const sessionElement = this.shadowRoot?.querySelector(`[data-session-id="${session.sessionId}"]`);
  ...
  private handleTestOptimisticUpdate(): void {
    ...
    this.webSocketController.optimisticUpdate('sessions', [...this.sessions, testSession]);
    ...
    return html`
      <div>
        ...
        <div class="actions">
          ...
          <button 
            class="action-button" 
            @click=${this.handleClearSessions}
          >
            Clear Sessions
          </button>
          <button 
          ...
          ${this.sessions.length > 0 
            ? this.sessions.map(session => html`
                <div class="session-item" data-session-id="${session.sessionId}">
                  ...
                  <small>
                    Created: ${session.createdAt ? new Date(session.createdAt).toLocaleString() : 'Unknown'}<br>
                    Updated: ${session.updatedAt ? new Date(session.updatedAt).toLocaleString() : 'Unknown'}<br>
                    Status: ${session.status || 'Unknown'}
                  </small>
                </div>
              `)
            : html`<p style="color: var(--color-text-muted); font-style: italic;">No sessions available</p>`
          }
        </div>
      </div>
    `;
