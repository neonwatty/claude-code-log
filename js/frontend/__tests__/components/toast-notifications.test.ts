...
jest.mock('../../src/utils/websocket/connection-state', () => ({
  ...
  getConnectionStateDisplay: jest.fn((state: string) => {
    const displays = {
      ...
      RECONNECTING: { label: 'Reconnecting', icon: '🔄', color: 'orange', severity: 'warning' },
      DISCONNECTED: { label: 'Disconnected', icon: '🔴', color: 'red', severity: 'error' },
...
class MockToastNotificationsComponent {
  ...
  showConnectionStateToast(event: ConnectionStateEvent) {
    ...
    const stateDisplay = displays[event.currentState as keyof typeof displays] || { label: 'Unknown', icon: '❓', color: 'gray', severity: 'info' };
...
describe('ToastNotificationsComponent', () => {
  ...
  describe('Component Lifecycle', () => {
    it('should create aria announcer on connection', () => {
      ...
      expect(mockAriaAnnouncer.setAttribute).toHaveBeenCalledWith('aria-live', 'polite');
      expect(mockAriaAnnouncer.setAttribute).toHaveBeenCalledWith('aria-atomic', 'true');
      expect(mockDocument.body.appendChild).toHaveBeenCalledWith(mockAriaAnnouncer);
    });
    ...
    it('should remove aria announcer on disconnection', () => {
      ...
      expect(mockDocument.body.removeChild).toHaveBeenCalledWith(mockAriaAnnouncer);
    });
  ...
  describe('Toast Management', () => {
    ...
    it('should clear all toasts', () => {
      component.showToast({ title: 'Toast 1', message: 'Message 1', type: 'info' });
      component.showToast({ title: 'Toast 2', message: 'Message 2', type: 'success' });
      component.showToast({ title: 'Toast 3', message: 'Message 3', type: 'warning' });
  ...
  describe('Accessibility', () => {
    ...
    it('should announce toast messages to screen readers', () => {
      ...
      expect(mockAriaAnnouncer.textContent).toBe('Important. Important message');
    });
  ...
  describe('Toast Types and Icons', () => {
    ...
    it('should handle all toast types correctly', () => {
      const types: Array<Toast['type']> = ['success', 'warning', 'error', 'info'];
