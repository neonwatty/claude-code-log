...
describe('SessionListWebSocketEnhanced', () => {
  ...
  beforeEach(async () => {
    ...
    (element as any).performOptimisticUpdate = vi.fn((sessionId: string, updates: any) => {
      ...
      realtimeUpdates.set(sessionId, { ...realtimeUpdates.get(sessionId), ...updates });
    ...
    mockController.onSessionUpdated((sessionData: SessionData, changes: any) => {
  ...
  describe('Component Initialization', () => {
    ...
    it('should setup WebSocket subscriptions', () => {
      ...
      expect(mockController.messageHandlers.has('cache-invalidated')).toBe(true);
    });
    ...
    it('should display realtime status indicator', async () => {
      ...
      const statusIndicator = element.shadowRoot!.querySelector('.realtime-status');
  ...
  describe('Connection State Management', () => {
    it('should display correct connection status', async () => {
      ...
      const statusElement = element.shadowRoot!.querySelector('.realtime-status');
    ...
    it('should show reconnect button when disconnected', async () => {
      ...
      const statusElement = element.shadowRoot!.querySelector('.realtime-status') as HTMLElement;
      ...
      const reconnectButton = element.shadowRoot!.querySelector('.reconnect-button');
  ...
  describe('Visual Feedback and Animations', () => {
    ...
    it('should show pending state for optimistic updates', async () => {
      (element as any).performOptimisticUpdate('session-1', { title: 'Pending Update' });
      await element.updateComplete;
  ...
  describe('Search and Filter Integration', () => {
    it('should handle search input changes', async () => {
      ...
      const searchInput = element.shadowRoot!.querySelector('.filter-input') as HTMLInputElement;
  ...
  describe('Error Handling', () => {
    ...
    it('should handle WebSocket controller errors gracefully', async () => {
      ...
      expect(() => {
        try {
          (element as any).performOptimisticUpdate('session-1', { title: 'Test' });
        } catch (error) {
  ...
  describe('Realtime Updates Disabled', () => {
    ...
    it('should not process WebSocket messages when disabled', async () => {
      ...
      expect(eventSpy).not.toHaveBeenCalledWith('session-created', expect.anything());
    });
  });
});