...
vi.mock("../../../src/utils/websocket/websocket-controller", () => {
  ...
  class MockWebSocketController {
    ...
    private _connectionState = "CONNECTED"; // Use string instead of enum to avoid hoisting issues
    ...
    withWebSocket: (Base: any, config?: any) => {
      return class extends Base {
        ...
        constructor(...args: any[]) {
          super(...args);
          this.webSocketController = new MockWebSocketController(
            this as any,
            undefined,
            config,
          );
        ...
        protected getWebSocketState(): any {
...
describe("withWebSocket HOC (Simplified)", () => {
  ...
  beforeEach(() => {
    ...
    class TestWebSocketComponent extends WebSocketEnabledComponent {
      setupWebSocketHandlers(): void {
        this.webSocketController.onSessionCreated(
  ...
  describe("HOC Factory", () => {
    it("should create a new class that extends the base component", () => {
      ...
      expect(WebSocketEnabledComponent.prototype).toBeInstanceOf(
        TestBaseComponent,
      );
    });
    ...
    it("should add WebSocket controller to the component", () => {
      ...
      expect(instance.webSocketController.constructor.name).toBe(
  ...
  describe("Enhanced Component Functionality", () => {
    ...
    it("should provide optimisticUpdate convenience method", () => {
      ...
      const mockController = instance.webSocketController as any;
      mockController.clearCallHistory();

      const testSessions = [
        { sessionId: "opt-test", title: "Optimistic Test" },
      ];
      ...
      expect(mockController.optimisticUpdateCalls[0].value).toEqual(
        testSessions,
      );
    });
  ...
  describe("WebSocket Integration", () => {
    ...
    it("should use updateFromWebSocket in message handlers", () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(
        "sessions",
        expect.arrayContaining([sessionData]),
      );
    });
  });
  ...
  describe("Configuration Inheritance", () => {
    it("should accept different configurations for different components", () => {
      ...
      expect(instance1.webSocketController.config).toEqual(
        expect.objectContaining(Config1),
      );
      expect(instance2.webSocketController.config).toEqual(
        expect.objectContaining(Config2),
      );
    });
  ...
  describe("Performance", () => {
    ...
    it("should not cause memory leaks with multiple instances", () => {
      ...
      const controllers = instances.map(
        (instance) => instance.webSocketController,
      );
