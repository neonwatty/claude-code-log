...
import type { ConnectionStatistics, ConnectionDebugInfo } from '../src/utils/websocket/connection-state';
...
class MockAppMain {
  ...
  private setupConnectionEventHandlers() {
    ...
    this.connectionManager.on('statistics-updated', (statistics: ConnectionStatistics) => {
      this.connectionStatistics = statistics;
    });

    this.connectionManager.on('debug-info-updated', (debugInfo: ConnectionDebugInfo) => {
  ...
  private handleConnectionStateChange(event: any) {
    ...
    mockAccessibilityService.announceConnectionState(event.currentState, event.reason);
  ...
  simulateDebugPanelToggle(visible: boolean) {
    this.handleDebugPanelToggled(new CustomEvent('debug-panel-toggled', {
      detail: { visible }
    }));
  }
}
...
describe('AppMain Integration Tests', () => {
  ...
  describe('Component Initialization', () => {
    ...
    it('should setup connection event handlers', async () => {
      ...
      expect(mockConnectionManager.on).toHaveBeenCalledWith('statistics-updated', expect.any(Function));
      expect(mockConnectionManager.on).toHaveBeenCalledWith('debug-info-updated', expect.any(Function));
    });
  ...
  describe('Connection State Management', () => {
    ...
    it('should update connection state when state changes', () => {
      app.simulateStateChange(ConnectionState.CONNECTED, 'Connection established');
    ...
    it('should announce state changes to accessibility service', () => {
      ...
      expect(mockAccessibilityService.announceConnectionState).toHaveBeenCalledWith(
  ...
  describe('User Interactions', () => {
    ...
    it('should connect when connect button clicked while disconnected', () => {
      mockConnectionManager.getConnectionState.mockReturnValue(ConnectionState.DISCONNECTED);
  ...
  describe('Toast Notification Integration', () => {
    ...
    it('should find toast notifications component in shadow root', () => {
      ...
      expect(app.shadowRoot.querySelector).toHaveBeenCalledWith('toast-notifications');
    ...
    it('should cache toast notifications component reference', () => {
      ...
      expect(mockToastComponent.showConnectionStateToast).toHaveBeenCalledTimes(2);
    });
  });
