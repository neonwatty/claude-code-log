...
router.post('/prepare-context', async (req: Request, res: Response) => {
  try {
    ...
    const contextResult = await claudeService.prepareSessionContext(request, foundSession);
...
router.post('/continue-with-context', async (req: Request, res: Response) => {
  try {
    // Validate request body
    const validation = SessionContinuationWithContextRequestSchema.safeParse(req.body);
    if (!validation.success) {
      const response: IApiResponse = {
        success: false,
    ...
    const continuationResponse = await claudeService.continueSessionWithContext(request, foundSession);
...
router.get('/:id/context', sessionIdValidation, handleValidationErrors, async (req: Request, res: Response) => {
  try {
    const validation = sessionIdSchema.safeParse(req.params.id);
    if (!validation.success) {
      const response: IApiResponse = {
    ...
    const contextResult = await claudeService.getSessionContextData(sessionId, foundSession);
...
router.delete('/:id/context', sessionIdValidation, handleValidationErrors, async (req: Request, res: Response) => {
  try {
    const validation = sessionIdSchema.safeParse(req.params.id);
    if (!validation.success) {
      const response: IApiResponse = {
    ...
    await claudeService.cleanupSessionContext(workingDirectory, keepClaudeMd || false);
