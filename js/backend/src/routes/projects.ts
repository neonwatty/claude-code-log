...
import { projectPathValidation, handleValidationErrors } from '../middleware/validation';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    for (const session of allSessions) {
      ...
      project.totalTokens += (session.totalUsage.input_tokens || 0) + (session.totalUsage.output_tokens || 0);
    }
...
router.get('/by-path/:encodedPath', async (req: Request, res: Response) => {
  try {
    ...
    const projectSessions = allSessions.filter(session => session.cwd === projectPath);
    ...
    const project: IProject = {
      ...
      path: projectPath,
      sessions: projectSessions.sort((a, b) => 
        new Date(b.lastTimestamp).getTime() - new Date(a.lastTimestamp).getTime()
      ),
      totalMessages: projectSessions.reduce((sum, session) => sum + session.entries.length, 0),
      totalTokens: projectSessions.reduce((sum, session) => 
        sum + (session.totalUsage.input_tokens || 0) + (session.totalUsage.output_tokens || 0), 0
      )
    };
