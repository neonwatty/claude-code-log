...
describe("Cache System Performance Tests", () => {
  ...
  describe("File Modification Service Performance", () => {
    it("should handle batch file checking efficiently", async () => {
      ...
      const filePaths = Array.from(
        { length: fileCount },
        (_, i) => `/test/project/file_${i.toString().padStart(4, "0")}.jsonl`,
      );
      ...
      await Promise.all(
        filePaths.map((fp) => services.fileModification.trackFile(fp)),
      );
      ...
      console.log(
        `File tracking: ${trackTime.toFixed(2)}ms for ${fileCount} files`,
      );
      console.log(
        `Batch checking: ${batchTime.toFixed(2)}ms for ${fileCount} files`,
      );
      console.log(`Average per file: ${(batchTime / fileCount).toFixed(2)}ms`);
    });

    it("should perform synchronous batch checking faster than async", async () => {
      const fileCount = 100;
      const filePaths = Array.from(
        { length: fileCount },
        (_, i) => `/test/project/sync_file_${i}.jsonl`,
      );
      ...
      // Mock fs.statSync for this test
      const fsModule = await import("fs");
      vi.mocked(fsModule.statSync).mockReturnValue(mockStats as any);
      ...
      console.log(
        `Sync batch checking: ${syncTime.toFixed(2)}ms for ${fileCount} files`,
      );
    });
  });
  ...
  describe("Cache Validation Performance", () => {
    it.skip("should validate large caches quickly", async () => {
        ...
        for (let i = 0; i < Math.ceil(sessionCount / 10); i++) {
          ...
          cache.cached_files[fileName] = {
            ...
            session_ids: Array.from(
              { length: Math.min(10, sessionCount - i * 10) },
              (_, j) => `session_${i * 10 + j}`,
            ),
          };
        }
      ...
      const result = await services.validation.validateCache(
        "/test/large-project",
      );
      ...
      console.log(
        `Large cache validation: ${validationTime.toFixed(2)}ms for 5000 sessions`,
      );
    });
    ...
    it.skip("should perform quick validation very fast", async () => {
      ...
      const result = await services.validation.quickValidate(
        "/test/quick-project",
      );
  ...
  describe("Cache Aggregation Performance", () => {
    it("should aggregate multiple large projects efficiently", async () => {
      ...
      console.log(
        `Stats calculation: ${statsTime.toFixed(2)}ms for ${projectCount} projects`,
      );
      console.log(
        `Summaries generation: ${summariesTime.toFixed(2)}ms for ${projectCount} projects`,
      );
      console.log(
        `Session query: ${queryTime.toFixed(2)}ms for ${stats.totalSessions} sessions`,
      );
    });
    ...
    it("should handle time-based aggregations efficiently", () => {
      ...
      const dailyAgg = services.aggregation.createTimeBasedAggregation(
        "day",
        "2023-01-01",
        "2023-01-31",
      );
      ...
      const hourlyAgg = services.aggregation.createTimeBasedAggregation(
        "hour",
        "2023-01-01",
        "2023-01-02",
      );
      ...
      console.log(
        `Hourly aggregation: ${hourlyTime.toFixed(2)}ms for 25 hours`,
      );
    });
  });
  ...
  describe("Memory Usage Performance", () => {
    it("should maintain reasonable memory usage under load", async () => {
      ...
      console.log(
        `Memory usage - Initial: ${initialMemory.heapUsed}MB, Peak: ${peakMemory.heapUsed}MB, Final: ${finalMemory.heapUsed}MB`,
      );
  ...
  describe("Concurrent Operations Performance", () => {
    it("should handle concurrent cache operations efficiently", async () => {
      ...
      const fileModificationPromises = Array.from(
        ...
        async (_, i) => {
          ...
          mockFs.stat.mockResolvedValue({
            size: 1024 + i,
            mtime: new Date(
              `2023-01-01T${(i % 24).toString().padStart(2, "0")}:00:00Z`,
            ),
          } as any);
      ...
      const successfulFileOps = fileResults.filter(
      ...
      console.log(
        `Concurrent operations: ${concurrentTime.toFixed(2)}ms for ${concurrentOperations * 2} operations`,
      );
      console.log(
        `Average per operation: ${(concurrentTime / (concurrentOperations * 2)).toFixed(2)}ms`,
      );
    });
  });
});
