...
describe("Streaming Parser Tests", () => {
  ...
  describe("interrupted file reads", () => {
    test("handles incomplete file reads gracefully", () => {
      const fullContent = [
        ...
        JSON.stringify({
          ...
          message: {
            role: "user",
            content: "Third message that will be cut off",
          },
        }),
      ].join("\n");

      // Simulate interrupted read by cutting off the last line
      const interruptedContent = fullContent.substring(
        0,
      ...
      if (result.entries[0].type === "user") {
        expect(result.entries[0].message.content).toBe(
          "First complete message",
        );
      }
      if (result.entries[1].type === "user") {
        expect(result.entries[1].message.content).toBe(
    ...
    test("handles file that ends mid-JSON gracefully", () => {
      const partialJson =
        '{"type":"user","uuid":"partial-uuid","timestamp":"2025-01-15T10:30:00.000Z","sessionId":"partial-session","message":{"role":"user","content":"This JSON will be cut o';
    ...
    test("does not accumulate excessive memory during long streams", () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const entry = {
          type: "user",
          uuid: `memory-${i}`,
          timestamp: `2025-01-15T${Math.floor(i / 3600)
            .toString()
            .padStart(2, "0")}:${Math.floor((i % 3600) / 60)
            .toString()
      ...
      console.log(
        `Max memory increase during streaming: ${maxMemoryIncrease.toFixed(2)}MB`,
      );
    });
  });
