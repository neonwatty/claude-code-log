...
describe("Error Handler Middleware", () => {
  ...
  describe("globalErrorHandler", () => {
    ...
    it("should handle JSON syntax errors with 400 status", async () => {
      const response = await request(app).get("/test?error=syntax").expect(400);
    ...
    it("should handle CORS errors with 403 status", async () => {
      const response = await request(app).get("/test?error=cors").expect(403);
    ...
    it("should include error details for AppErrors", async () => {
      ...
      const response = await request(app).get("/detailed").expect(400);
  ...
  describe("asyncHandler", () => {
    it("should catch async errors and pass to error handler", async () => {
      ...
      const response = await request(app).get("/async-test").expect(400);
    ...
    it("should handle rejected promises", async () => {
      ...
      const response = await request(app).get("/async-reject").expect(500);
  ...
  describe("requestLogger", () => {
    ...
    it("should log request and response details", async () => {
      ...
      await request(app).get("/logged-route").expect(200);
    ...
    it("should measure request duration", async () => {
      ...
      await request(app).get("/slow-route").expect(200);
  ...
  describe("Error Response Format", () => {
    beforeEach(() => {
      app.use("/format-test", (req, res, next) => {
        next(
          new ValidationError("Format test error", { additionalInfo: "test" }),
        );
      });
      app.use(globalErrorHandler);
    });
    ...
    it("should include all required response fields", async () => {
      const response = await request(app).get("/format-test").expect(400);
    ...
    it("should omit details when not provided", async () => {
      ...
      const response = await request(app).get("/no-details").expect(404);
