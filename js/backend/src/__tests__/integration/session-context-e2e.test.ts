...
describe('Session Context Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testSessionFile = path.join(process.cwd(), `test-session-${Date.now()}.jsonl`);
    ...
    const jsonlContent = sessionEntries.map(entry => JSON.stringify(entry)).join('\n');
  ...
  beforeEach(async () => {
    ...
    const mockedModule = await import('../../services/claude-integration.service.js') as any;
  ...
  describe('GET /api/sessions/:id/context', () => {
    it('should retrieve session context data', async () => {
      ...
      expect(contextData.codePatterns.modifiedFiles).toContain(path.join(testDir, 'auth-service.ts'));
    });
    ...
    it('should prepare session context and generate CLAUDE.md', async () => {
      const contextRequest: ContextPreparationRequest = {
        ...
        config: {
          ...
          additionalInstructions: 'This is a test authentication service project',
        },
      };
      ...
      expect(contextResult.claudeMdContent).toContain('This is a test authentication service project');
  ...
  describe('DELETE /api/sessions/:id/context', () => {
    ...
    it('should clean up context files', async () => {
      ...
      const existsBeforeCleanup = await fs.access(claudeMdPath).then(() => true).catch(() => false);
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.message).toBe('Context cleanup completed successfully');
    ...
    it('should keep CLAUDE.md when requested', async () => {
      ...
      const existsAfterCleanup = await fs.access(claudeMdPath).then(() => true).catch(() => false);
    ...
    it('should return 400 when working directory is missing', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error).toBe('Working directory is required for context cleanup');
    });
  ...
  describe('Error handling and edge cases', () => {
    it('should handle sessions with minimal data', async () => {
      ...
      const minimalSessionFile = path.join(process.cwd(), `minimal-session-${Date.now()}.jsonl`);
      ...
      await fs.writeFile(minimalSessionFile, minimalEntries.map(e => JSON.stringify(e)).join('\n'), 'utf-8');
