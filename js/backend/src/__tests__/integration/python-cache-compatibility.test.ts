...
describe("Python Cache Compatibility Integration", () => {
  ...
  describe("Python-generated Cache Compatibility", () => {
    const createPythonGeneratedCache = () => ({
      ...
      sessions: {
        ...
        ghi789: {
          ...
          message_count: 18,
          first_user_message:
            "What are the best practices for cache performance?",
          cwd: "/home/user/projects/python-cache",
    ...
    it.skip("should successfully read and validate Python-generated cache", async () => {
      ...
      const validationResult = await cacheValidationService.validateCache(
        testProjectPath,
        {
    ...
    it("should handle Python cache aggregation correctly", async () => {
      ...
      aggregationService["aggregatedCache"].set(
        testProjectPath,
        pythonCache as any,
      );
      ...
      expect(aggregatedStats.averageMessagesPerSession).toBe(
        Math.round(43 / 3),
      );
    });
    ...
    it("should query Python cache sessions correctly", async () => {
      ...
      expect(sessions.every((s) => s.projectPath === testProjectPath)).toBe(
        true,
      );
    });
  });
  ...
  describe("Legacy Python Cache Migration", () => {
    ...
    it.skip("should migrate 0.9.0 Python cache to current version", async () => {
      ...
      const result = await cacheValidationService.validateCache(
        testProjectPath,
        {
          enableVersionMigration: true,
        },
    ...
    it.skip("should migrate 0.8.0 Python cache to current version", async () => {
      ...
      expect(migratedData.sessions.legacy456.total_cache_creation_tokens).toBe(
        0,
      );
  ...
  describe("Python Cache Structure Validation", () => {
    it("should validate Python cache with snake_case field names", async () => {
      ...
      const validationResult = (
        cacheValidationService as any
      ).validateStructure(pythonStyleCache);
    ...
    it("should detect Python cache corruption", async () => {
      ...
      const validationResult = (
        cacheValidationService as any
      ).validateStructure(corruptedPythonCache);

      expect(validationResult.isValid).toBe(false);
      expect(validationResult.reason).toContain(
        "Invalid cached_files field type",
      );
    });
  });
  ...
  describe("Cross-language Token Calculations", () => {
    it("should calculate totals consistently with Python implementation", async () => {
      ...
      const expectedInputTokens = Object.values(testCache.sessions).reduce(
        (sum, session) => sum + session.total_input_tokens,
        0,
      );
      const expectedOutputTokens = Object.values(testCache.sessions).reduce(
        (sum, session) => sum + session.total_output_tokens,
        0,
      );
      const expectedCacheCreationTokens = Object.values(
        testCache.sessions,
      ).reduce((sum, session) => sum + session.total_cache_creation_tokens, 0);
      const expectedCacheReadTokens = Object.values(testCache.sessions).reduce(
        (sum, session) => sum + session.total_cache_read_tokens,
        0,
      );
      ...
      expect(stats.averageTokensPerMessage).toBe(
        Math.round(
          (expectedInputTokens + expectedOutputTokens) / stats.totalMessages,
        ),
      );
    });
  });
    ...
    it("should handle large Python-generated caches efficiently", async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        largePythonCache.sessions[sessionId] = {
          ...
          first_timestamp: `2023-01-01T${Math.floor(i / 42)
            .toString()
            .padStart(2, "0")}:${(i % 60).toString().padStart(2, "0")}:00.000Z`,
          last_timestamp: `2023-01-01T${Math.floor(i / 42)
            .toString()
            .padStart(
              2,
              "0",
            )}:${((i % 60) + 1).toString().padStart(2, "0")}:00.000Z`,
      ...
      // Calculate totals
      largePythonCache.total_message_count = Object.values(
        largePythonCache.sessions,
      ).reduce(
        (sum: number, session: any) => sum + session.message_count,
        0,
      ) as number;
      largePythonCache.total_input_tokens = Object.values(
        largePythonCache.sessions,
      ).reduce(
        (sum: number, session: any) => sum + session.total_input_tokens,
        0,
      ) as number;

      aggregationService["aggregatedCache"].set(
        testProjectPath,
        largePythonCache as any,
      );
