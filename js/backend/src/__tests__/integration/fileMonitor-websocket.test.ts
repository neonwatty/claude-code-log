...
const mockGetWebSocketManager = getWebSocketManager as MockedFunction<
...
describe("FileMonitor + WebSocket Integration", () => {
  ...
  describe("Performance Considerations", () => {
    ...
    it("should handle large JSONL files efficiently", async () => {
      ...
      const totalCalls =
        (wsManager.broadcastSessionCreated as any).mock.calls.length +
  ...
  describe("Event Ordering", () => {
    ...
    it("should maintain correct event order for file and session events", async () => {
      ...
      const fileChangedOrder = (wsManager.broadcastFileChanged as any).mock
        .invocationCallOrder[0];
      const sessionCreatedOrder = (wsManager.broadcastSessionCreated as any)
