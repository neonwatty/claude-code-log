...
describe("JSONL Parser Validation and Error Handling", () => {
    ...
    test("handles JSON with special characters and unicode", () => {
      const specialChars = {
        ...
        message: {
          role: "user",
          content:
            "Testing unicode: 🚀 emoji, \"quotes\", 'single quotes', \\backslashes\\, \nnewlines\n, \ttabs\t, and null\0chars",
        },
      };
  ...
  describe("parseTranscriptEntry validation", () => {
    ...
    test("validates assistant message structure", () => {
      ...
      expect(() =>
        parseTranscriptEntry(assistantMessageWithContent),
      ).not.toThrow();
    });
  ...
  describe("timestamp validation", () => {
    ...
    test("handles various timestamp formats", () => {
      ...
      timestampVariations.forEach((timestamp, _index) => {
        const entry = {
          type: "user",
          uuid: "timestamp-test",
          timestamp,
