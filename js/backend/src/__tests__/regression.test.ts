...
import { parseJsonlLine, parseTranscriptEntry, loadTranscript } from '../parsers/jsonl-parser';
...
describe('Regression Tests for Known Parsing Issues', () => {
  ...
  describe('Issue #1: Malformed content field in tool_result', () => {
    test('handles tool_result with typo in content field', () => {
      ...
      const malformedToolResult = {
        ...
        message: {
          role: 'user',
          contenst: [{ // typo: "contenst" instead of "content"
            type: 'tool_result',
      ...
      const mixedContentEntry = {
        ...
        message: {
          ...
          content: [
            ...
            { type: 'thinking', thinking: 'I need to analyze the user request carefully.', signature: 'sig_123' },
    ...
    test('handles tool use with deeply nested input parameters', () => {
      ...
      if (result.type === 'assistant' && Array.isArray(result.message.content)) {
        ...
        if (toolUse.type === 'tool_use' && 'input' in toolUse) {
          expect(toolUse.input.config.nested.deeply.params.array).toEqual([1, 2, 3, { key: 'value' }]);
          expect(toolUse.input.config.nested.deeply.params.unicode).toBe('🚀 emojis and 中文 characters');
        }
      }
    });
  });
  ...
  describe('Issue #6: Special characters in content causing JSON parse errors', () => {
    test('handles escaped special characters correctly', () => {
      const specialContent = 'Text with "quotes", \'single quotes\', \\backslashes\\, \nnewlines\n, \ttabs\t, and unicode: 🚀 emoji';
    ...
    test('handles null bytes and control characters', () => {
      const controlCharsContent = 'Text with\x00null\x01control\x02chars\x03and\x1Fmore';
