...
describe("JSONL Parser Performance", () => {
  ...
  test("parses large file in reasonable time", () => {
    ...
    const sampleEntry = {
      ...
      message: {
        role: "user",
        content:
          "This is test message number {{INDEX}} with some content that simulates real user input. " +
    ...
    console.log(
      `Parsed 1000 entries in ${durationMs.toFixed(2)}ms (${((1000 / durationMs) * 1000).toFixed(0)} entries/sec)`,
    );
  });
  ...
  test("handles memory efficiently with many small messages", () => {
    ...
    for (let i = 0; i < 5000; i++) {
      const entry = {
        type: "assistant",
        uuid: `msg-${i}`,
        timestamp: `2025-01-15T10:${Math.floor(i / 60)
          .toString()
  ...
  test("gracefully handles corrupted large files", () => {
    ...
    console.log(
      `Parsed file with 10% corruption in ${durationMs.toFixed(2)}ms`,
    );
  });
