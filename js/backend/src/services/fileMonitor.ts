...
export class FileMonitor extends EventEmitter {
  ...
  private debouncedFileChange(type: 'created' | 'modified' | 'deleted', filePath: string): void {
  ...
  private processFileChange(type: 'created' | 'modified' | 'deleted', filePath: string): void {
    try {
  ...
  private parseJsonlForSessionChanges(filePath: string): void {
    try {
      ...
      const sessionSummary = new Map<string, {
        sessionId: string;
        cwd: string;
        entryCount: number;
